...
RtspConnection::RtspConnection(RtspServer* rtspServer, int sockfd) :
    TcpConnection(rtspServer->getEventScheduler()->getPoller(), sockfd),
    mRtspServer(rtspServer), mMethod(NONE), mTrackId(TrackIdNone),
    mSessionId(rand()), mIsRtpOverTcp(false)
{
...
void RtspConnection::handleReadBytes()
{
    ...
    ret = parseRequest();
    if(ret != true) {
        LOG_WARN("failed to parse request\n");
        goto err;
    }
...
{
    snprintf(mBuffer, sizeof(mBuffer), "RTSP/1.0 200 OK\r\nCSeq: %u\r\nPublic: OPTIONS, DESCRIBE, SETUP, TEARDOWN, PLAY\r\n\r\n", mCSeq);
...
bool RtspConnection::handleCmdDescribe()
{
    ...
    if(!session) {
        LOG_WARN("can't loop up %s session\n", mSuffix.c_str());
        return false;
    }
...
bool RtspConnection::handleCmdSetup()
{
    ...
    if(session->isStartMulticast()) {
        snprintf((char*)mBuffer, sizeof(mBuffer), "RTSP/1.0 200 OK\r\nCSeq: %d\r\nTransport: RTP/AVP;multicast;"
        ...
        if(mIsRtpOverTcp) { //rtp over tcp
            ...
            if(createRtpRtcpOverUdp(mTrackId, mPeerIp, mPeerRtpPort, mPeerRtcpPort) != true) {
                LOG_WARN("failed to create rtp and rtcp\n");
                return false;
            }
