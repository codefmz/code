...
RtspServer* RtspServer::createNew(EventScheduler* scheduler, Ipv4Address& addr)
{
    return new RtspServer(scheduler, addr);
}

RtspServer::RtspServer(EventScheduler* scheduler, const Ipv4Address& addr) :
    TcpServer(scheduler->getPoller(), addr), mScheduler(scheduler)
{
...
void RtspServer::handleDisconnection(int sockfd)
{
    ...
    mScheduler->addTriggerEvent(mTriggerEvent);
}
