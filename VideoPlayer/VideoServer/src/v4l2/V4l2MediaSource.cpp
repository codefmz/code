...
V4l2MediaSource* V4l2MediaSource::createNew(ThreadPool* pool, std::string dev)
{
    return new V4l2MediaSource(pool, dev);
}

V4l2MediaSource::V4l2MediaSource(ThreadPool *pool, const std::string& dev) :
    MediaSource(pool),
    mPool(pool),
    mDev(dev),
    mWidth(640),
...
void V4l2MediaSource::readFrame()
{
    ...
    if (mNaluQueue.empty()) {
        ...
        while (1) {
            ...
            if (ret< 0) {
                LOG_WARN("failed to encode data\n");
                return;
            }
...
bool V4l2MediaSource::x264Init()
{
    ...
    if(!mX264Handle) {
        LOG_ERR("failed to open x264 encoder\n");
        return false;
    }
