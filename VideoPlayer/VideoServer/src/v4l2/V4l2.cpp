...
{
    if (close(fd)) {
        LOG_ERR("failed to close v4l2 dev");
        return -1;
    }
...
int v4l2_querycap(int fd, struct v4l2_capability* cap)
{
    if (ioctl(fd, VIDIOC_QUERYCAP, cap) < 0) {
        LOG_ERR("VIDIOC_QUERYCAP failed");
        return -1;
    }
...
int v4l2_poll(int fd)
{
    ...
    if (ret == 0)
    {
        printf("ERR(%s):No data in 100 secs..\n", __func__);
        return -1;
    }
