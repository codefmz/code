...
QVariant TreeModel::data(const QModelIndex &index, int role) const
{
    ...
    if (TaskItem *item = itemForIndex(index)) {
        ...
        if (role == Qt::CheckStateRole && index.column() == Name) {
            return static_cast<int>(item->isDone() ? Qt::Checked : Qt::Unchecked);
        }
        ...
        if (role == Qt::DecorationRole && index.column() == Today && timedItem && item == timedItem && !m_icon.isNull()) {
            return m_icon;
        }
    }
    return QVariant();
...
{
    if (index.isValid()) {
        if (TaskItem *item = static_cast<TaskItem*>(index.internalPointer())) {
            return item;
        }
    }
    return rootItem;
...
bool TreeModel::insertRows(int row, int count, const QModelIndex &parent)
{
    ...
    TaskItem *parentItem = parent.isValid() ? itemForIndex(parent) : rootItem;
...
void TreeModel::announceItemChanged(TaskItem *item)
{
    ...
    QModelIndex endIndex = createIndex(row, static_cast<int>(Total), item);
    emit dataChanged(startIndex, endIndex);
    // Update the parent & parent's parent etc too
    announceItemChanged(parent);
}

void TreeModel::addDateTimeToTimedItem(const QDateTime &start, const QDateTime &end)
{
    if (timedItem) {
    ...
    while (!reader->atEnd()) {
        ...
        if (reader->isStartElement()) {
            if (reader->name() == TaskTag) {
                ...
                const QDateTime start = QDateTime::fromString(reader->attributes().value(StartAttribute).toString(), Qt::ISODate);
                const QDateTime end = QDateTime::fromString(reader->attributes().value(EndAttribute).toString(), Qt::ISODate);
                Q_ASSERT(task);
...
void TreeModel::writeTaskAndChildren(QXmlStreamWriter *writer, TaskItem *task) const
{
    if (task != rootItem) {
        ...
        while (i.hasNext()) {
            ...
            writer->writeAttribute(StartAttribute, dateTime.first.toString(Qt::ISODate));
            writer->writeAttribute(EndAttribute, dateTime.second.toString(Qt::ISODate));
...
QModelIndex TreeModel::indexForPath(const QModelIndex &parent, const QStringList &path) const
{
    if (path.isEmpty()) {
        return QModelIndex();
    }
