...
RichTextLineEdit::RichTextLineEdit(QWidget *parent) : QTextEdit(parent)
{
...
{
    QShortcut *boldShortcut = new QShortcut(QKeySequence::Bold, this, SLOT(toggleBold()));
    QShortcut *italicShortcut = new QShortcut(QKeySequence::Italic, this, SLOT(toggleItalic()));

    setToolTip(tr("<p>Use %1 to toggle bold, %2 to toggle italic, "
        "and the context menu for color and other effects.")
        .arg(boldShortcut->key().toString(QKeySequence::NativeText))
        .arg(italicShortcut->key().toString(QKeySequence::NativeText)));
}
...
void RichTextLineEdit::createActions()
{
    ...
    noSubOrSuperScriptAction = createAction(tr("No super or subscript"), NoSuperOrSubscript);
    ...
    addActions(QList<QAction*>() << boldAction << italicAction << strikeOutAction << noSubOrSuperScriptAction
        << superScriptAction << subScriptAction << colorAction);
//    AQP::accelerateActions(actions());
}

QAction *RichTextLineEdit::createAction(const QString &text, const QVariant &data)
{
...
QMenu *RichTextLineEdit::createColorMenu()
{
    ...
    connect(colorMenu, SIGNAL(triggered(QAction*)), this, SLOT(applyColor(QAction*)));
...
void RichTextLineEdit::updateContextMenuActions()
{
    ...
    noSubOrSuperScriptAction->setChecked(format.verticalAlignment() == QTextCharFormat::AlignNormal);
    superScriptAction->setChecked(format.verticalAlignment() == QTextCharFormat::AlignSuperScript);
    subScriptAction->setChecked(format.verticalAlignment() == QTextCharFormat::AlignSubScript);
}
