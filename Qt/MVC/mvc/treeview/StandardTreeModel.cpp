...
{
    setHorizontalHeaderLabels(QStringList() << tr("Task/Subtask/...") << tr("Time (Today)") << tr("Time (Total)"));
    ...
    while (!reader.atEnd()) {
        ...
        if (reader.isStartElement()) {
            if (reader.name() == TaskTag) {
                ...
                StandardItem *nameItem = static_cast<StandardItem*>(stack.top());
...
void StandardTreeModel::writeTaskAndChildren(QXmlStreamWriter *writer, QStandardItem *root)
{
    if (root != invisibleRootItem()) {
        StandardItem *item = static_cast<StandardItem*>(root);
        writer->writeStartElement(TaskTag);
        writer->writeAttribute(NameAttribute, item->text());
        writer->writeAttribute(DoneAttribute, item->checkState() == Qt::Checked ? "1" : "0");
        QListIterator<QPair<QDateTime, QDateTime> > i(item->dateTimes());
        while (i.hasNext()) {
